...
This law has two parameters : V and $\beta$. Using a HOBO device we measured for two sensors at the same temperature the voltage for one and the temperature for the other and then by using a linear regression between $ln(v_{mes}/(v-v_{mes}))$ and $1/T$ we obtained $\beta$. The parameter V is given by the electronic circuit to which the sensor is connected. Indeed, it delivers a voltage of 3.3V, so $V=3.3$. 

You can find in the same folder the Python file (.py) used to clean the csv datas and create a new csv with only the useful datas, that was then used in an Excel file to determine the $\beta$ parameter with its uncertainty, thanks to the DROITEREG function. 
