...
- Platformio fonctionne bien et permet la gestion de bibliothèques externes et internes au projet. La gestion du fichier platformio.ini dépend de la suite du projet mais elle peut être grandement conservée.
...
- On arrive à envoyer des données du relay à la gateway et de la gateway au serveur et du serveur au site. (avec les codes démo qui sont en gros les mêmes avec des temps différents, faites la différence avec les mains et corrigez les pour le vrai fonctionnement).
...
- Forme d'optimisation des bibliothèques pour les rendre plus claires : petites bibliothèques avec seulement les fonctions de base, ne s'appelant pas les unes les autres. Plus grosses librairies qui les appellent par groupe pour former les blocs important, qu'on appelle ensuite dans les main. Ce n'est qu'une idée pas nécessaire
- Comme vous pourrez voir dans le paragraphe d'au-dessous que la gestion du temps n'est pas satisfaisante. Pour ne plus avoir de problème de décalage des fenêtres de communication il faut:
    ...
    NOTA TRES BENE : une fois les fichiers upload n'appuyer plus sur reset ca décale les horloges : elles se remettent à l'heure du dernier upload)
...
- C'est très galère d'upload sur les cartes avec un Mac faite le avec un Windows, à la fin tout marchait sur mac mais bon.
...
- On utilise platformio pour upload et arduino ide pour visualiser ce que nous dit la carte en temps réel (println, DEBUG_LOg...)
