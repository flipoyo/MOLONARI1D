...
void LoraCommunication::sendPacket(uint8_t packetNumber, RequestType requestType, const String &payload) {
    ...
    DEBUG_LOG("checksum =" + String(checksum));
    LoRa.write(checksum);
    LoRa.write(buffer_localAddress, localAddress.length());
    LoRa.write(buffer_destination, destination.length());
    LoRa.write(packetNumber); //envoie d'un seul octet, pas besoin de la taille
...
bool LoraCommunication::receivePacket(uint8_t &packetNumber, RequestType &requestType, String &payload) {
    ...
    while (millis() - startTime < ackTimeout) {
        ...
        if (packetSize) {
            ...
            DEBUG_LOG("recipient est :"+ String (recipient));
            DEBUG_LOG("sender est :"+String(sender));

            if (!isValidDestination(sender, recipient, requestType)) {
...
bool LoraCommunication::isValidDestination(const String &sender, const String &recipient, RequestType requestType) {
    if (sender != localAddress) return false;
    if (destination == recipient || (requestType == SYN && destination == String(0xff) && myNet.find(recipient.toInt()) != myNet.end())) {
