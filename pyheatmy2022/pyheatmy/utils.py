...
def compute_T(
    ...
    for j, dt in enumerate(all_dt):
        ...
        lower_diagonal = (ke * alpha / dz**2) - (alpha * ae / (2 * dz)) * nablaH[
            1:, j
        ]
        lower_diagonal[-1] = (
            4 * ke * alpha / (3 * dz**2)
            - (2 * alpha * ae / (3 * dz)) * nablaH[n_cell - 1, j]
        )
        ...
        upper_diagonal = (ke * alpha / dz**2) + (alpha * ae / (2 * dz)) * nablaH[
            :-1, j
        ]
        upper_diagonal[0] = (
            4 * ke * alpha / (3 * dz**2) + (2 * alpha * ae / (3 * dz)) * nablaH[0, j]
        )
        ...
        c[0] = (
            8 * ke * (1 - alpha) / (3 * dz**2)
            - 2 * (1 - alpha) * ae * nablaH[0, j] / (3 * dz)
        ) * T_riv[j + 1] + (
            8 * ke * alpha / (3 * dz**2) - 2 * alpha * ae * nablaH[0, j] / (3 * dz)
        ) * T_riv[
            j
        ]
        c[-1] = (
            8 * ke * (1 - alpha) / (3 * dz**2)
            + 2 * (1 - alpha) * ae * nablaH[n_cell - 1, j] / (3 * dz)
        ) * T_aq[j + 1] + (
            8 * ke * alpha / (3 * dz**2)
            + 2 * alpha * ae * nablaH[n_cell - 1, j] / (3 * dz)
        ) * T_aq[
            j
        ]
...
@njit
def compute_H(
    moinslog10K, Ss, all_dt, isdtconstant, dz, H_init, H_riv, H_aq, alpha=ALPHA
):
    ...
    if isdtconstant:  # dt is constant so A and B are constant
        ...
        # Defining the 3 diagonals of A
        lower_diagonal_A = full(n_cell - 1, -KsurSs * (1 - alpha) / dz**2, float32)
        ...
        upper_diagonal_A = full(n_cell - 1, -KsurSs * (1 - alpha) / dz**2, float32)
        ...
        for j, dt in enumerate(all_dt):
            ...
            # Defining the 3 diagonals of A
            lower_diagonal = full(n_cell - 1, -KsurSs * (1 - alpha) / dz**2, float32)
            lower_diagonal[-1] = -4 * KsurSs * (1 - alpha) / (3 * dz**2)

            diagonal = full(
                n_cell, 1 / dt + 2 * KsurSs * (1 - alpha) / dz**2, float32
            )
            diagonal[0] = 1 / dt + 4 * KsurSs * (1 - alpha) / dz**2
            diagonal[-1] = 1 / dt + 4 * KsurSs * (1 - alpha) / dz**2

            upper_diagonal = full(n_cell - 1, -KsurSs * (1 - alpha) / dz**2, float32)
