...
class Column:  # colonne de sédiments verticale entre le lit de la rivière et l'aquifère
    ...
    def compute_mcmc_without_sigma2(
        ...
        dz = self._real_z[-1] / nb_cells  # pas spatial
        _z_solve = dz / 2 + np.array(
            [k * dz for k in range(nb_cells)]
        )  # discrétisation spatiale
        ind_ref = [
            np.argmin(np.abs(z - _z_solve)) for z in self._real_z[1:-1]
        ]  # indice représentant la position des mesures
        temp_ref = self._T_measures[:, :].T  # température de référence
        ...
        self._states = list()  # stockage des paramètres
        _temps = np.zeros(
            (nb_iter + 1, nb_cells, len(self._times)), np.float32
        )  # stockage des températures
        _flows = np.zeros(
            (nb_iter + 1, nb_cells, len(self._times)), np.float32
        )  # stockage des débits spécifiques

        for _ in trange(1000, desc="Init Mcmc ", file=sys.stdout):
            init_layers = all_priors.sample()  # échantillonne les paramètres
    ...
    def compute_dream_mcmc_without_sigma2(
        ...
        _energy_burn_in = np.zeros((nb_iter + 1, nb_chain))  # énergie pour le burn-in

        # variables pour l'état courrant
        temp_new = np.zeros((nb_cells, len(self._times)))  # température calculée
        energy_new = 0  # énergie calculée
        ...
        self._acceptance = np.zeros((nb_chain,))  # stockage des taux d'acceptation

        _params = np.zeros(
            (nb_iter + 1, nb_chain, nb_layer, nb_param)
        )  # stockage des paramètrs
        ...
        # Préparation du sous-échantillonage
        ...
        _temp = np.zeros(
            (nb_iter + 1, nb_chain, nb_cells_sous_ech, nb_times_sous_ech), np.float32
        ...
        _propose_params = np.zeros((nb_iter, nb_chain, nb_layer, nb_param))
        for i in trange(nb_iter, desc="DREAM MCMC Computation", file=sys.stdout):
            ...
            for j in range(nb_chain):
                ...
                _propose_params[i, j] = x_new  # stockage des paramètres proposés
                # Calcul du profil de température associé aux nouveaux paramètres
                self.compute_solve_transi(
                    convert_to_layer(nb_layer, name_layer, z_low, x_new),
                    nb_cells,
    ...
    def compute_dream_mcmc_para(
        ...
        for i in trange(nb_iter):
            ...
            def perturb_chain(chain):
                ...
                for l in range(nb_layer):
                    ...
                    lambd = np.random.uniform(-c, c, d_star)
                    zeta = np.random.normal(0, c_star, d_star)
                    choose = np.delete(np.arange(nb_chain), chain)
                    a = np.random.choice(choose, delta, replace=False)
                    ...
                    x_new[l] = (
                        X[chain, l] + dX[l]
                    )  # caclul du potentiel nouveau paramètre
                    x_new[l] = check_range(
                        x_new[l], ranges[l]
                    )  # vérification des bornes et réajustement si besoin
                # Calcul du profil de température associé aux nouveaux paramètres
                ...
                return id, dX, x_new, temp_new, energy_new

            # results = Parallel(n_jobs=5)(
            #     delayed(perturb_chain)(j) for j in range(nb_chain)
            # )

            for j in range(nb_chain):
                # id, dX, x_new, temp_new, energy_new = results[j]
                id, dX, x_new, temp_new, energy_new = perturb_chain(j)
        ...
        for i in trange(nb_iter, desc="DREAM MCMC Computation", file=sys.stdout):
            ...
            def perturb_chain(chain):
                ...
                flows_new = self.get_flows_solve()
                energy_new = compute_energy(
                    temp_new[ind_ref, :], temp_ref, sigma2
                )  # calcul de l'énergie

                return id, dX, x_new, temp_new, flows_new, energy_new
            ...
            for j in range(nb_chain):
                # id, dX, x_new, temp_new, flows_new, energy_new = results[j]
                id, dX, x_new, temp_new, flows_new, energy_new = perturb_chain(j)
                # calcul de la probabilité d'accpetation
