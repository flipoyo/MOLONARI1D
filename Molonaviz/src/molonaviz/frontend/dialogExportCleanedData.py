...
From_DialogExportCleanedData = uic.loadUiType(get_ui_asset("dialog.ui"))[0]


class DialogExportCleanedData(QtWidgets.QDialog, From_DialogExportCleanedMeasures):
    """
    ...
    def accept(self):
        ...
        if not os.path.isdir(self.lineEditCleanMeasDir.text()):
            displayCriticalMessage(
                "This directory does not exist. Please select another one."
            )
            return
        super().accept()
