...
class DialogCleanup(QtWidgets.QDialog, From_DialogCleanup):
    """
    A dialog to either automatically clean the raw measures, or to import cleaned measures.
    ...
    def __init__(
        self, coordinator: SPointCoordinator, spoint: SamplingPoint
        ...
        self.radioButtonNone.clicked.connect(
            self.setNoneComputation
        )  # This should already be done in the UI
    ...
    def buildDF(self):
        ...
        self.data = pd.DataFrame(
            backend_data,
            columns=["Date", "Temp1", "Temp2", "Temp3", "Temp4", "TempBed", "Voltage"],
        )
    ...
    def convertVoltagePressure(self):
        ...
        """
        self.data["Pressure"] = (
            self.data["Voltage"] - self.data["TempBed"] * self.dUdT - self.intercept
        ) / self.dUdH
    ...
    def setupStartEndDates(self):
        ...
        for spin in [
            self.spinBoxStartDay,
            self.spinBoxStartMonth,
            self.spinBoxStartYear,
            self.spinBoxEndDay,
            self.spinBoxEndMonth,
            self.spinBoxEndYear,
        ]:
    ...
    def applyDateBoundariesChanges(self, cleanedData: pd.DataFrame):
        ...
        """
        pd_startDate = pd.Timestamp(
            day=self.spinBoxStartDay.value(),
        ...
        pd_endDate = pd.Timestamp(
            day=self.spinBoxEndDay.value(),
    ...
    def applyTemperatureChanges(self, cleanedData: pd.DataFrame):
        ...
        if self.radioButtonC.isChecked():
            ...
            referenceData[
                ["Temp1", "Temp2", "Temp3", "Temp4", "TempBed"]
            ] = referenceData[["Temp1", "Temp2", "Temp3", "Temp4", "TempBed"]].apply(
                convertFun
            )
            cleanedData[["Temp1", "Temp2", "Temp3", "Temp4", "TempBed"]] = cleanedData[
                ["Temp1", "Temp2", "Temp3", "Temp4", "TempBed"]
            ].apply(convertFun)
    ...
    def browse(self):
        filePath = QtWidgets.QFileDialog.getOpenFileName(
            self, "Get Cleaned Measures File", "", "CSV files (*.csv)"
        )[0]
        if filePath:
    ...
    def importCleanedData(self, path):
        ...
        if columnnames != [
            ...
            displayCriticalMessage(
                "The columns must be named (in this order): Date, Temp1, Temp2, Temp3, Temp4, TempBed, Pressure."
            )
        ...
        if not (pd.Index(np.arange(0, len(df))).equals(df.index)):
            displayCriticalMessage(
                "The dataframe must have the default index column as a numbering method."
            )
            raise InvalidCSVStructure
        ...
        try:
            ...
            displayCriticalMessage(
                "The 'Date' column can't be converted to datetime objects."
            )
            raise InvalidCSVStructure
        return df
    ...
    def getCleanedMeasures(self):
        ...
        if pathToCleaned == "":
            ...
            df_to_keep = self.data.merge(
                cleaned_u_selected,
                on=["Date", "Temp1", "Temp2", "Temp3", "Temp4", "TempBed", "Pressure"],
                how="left",
                indicator=True,
            )
