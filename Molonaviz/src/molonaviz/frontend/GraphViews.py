    ...
    def plotData(self):
        if self.cmap.shape[1] == len(self.x) and self.cmap.shape[0] == len(self.y):
            ...
            image = self.axes.imshow(
                ...
                cmap=cm.Spectral_r,
                aspect="auto",
                extent=[self.x[0], self.x[-1], float(self.y[-1]), float(self.y[0])],
                data="float",
            )
...
class GraphViewHisto(GraphView):
    ...
    def __init__(
        self, molomodel: MoloModel | None, bins=60, color="green", title="", xlabel=""
    ):
        super().__init__(molomodel)
        self.bins = bins
        self.data = []
...
class PressureView(GraphView1D):
    ...
    def retrieveData(self):
        ...
        self.y = {
            "": np.float64(self.model.get_pressure())
        }  # No label required for this one.
...
class TemperatureView(GraphView1D):
    ...
    def retrieveData(self):
        self.x = self.model.get_dates()
        self.y = {
            f"Sensor n°{i}": np.float64(temp)
            for i, temp in enumerate(self.model.get_temperatures())
        }
    ...
    def retrieveData(self):
        if self.options[0] is not None:  # A computation has been done.
            ...
            for quantile in self.options[1]:
                self.y[
                    f"Temperature at depth {depth_thermo:.3f} m - quantile {quantile}"
                ] = self.model.get_temp_by_date(depth_thermo, quantile)
...
class Log10KView(GraphViewHisto):
    ...
    def __init__(
        ...
        bins=60,
        color="green",
        title="A posteriori histogram of the permeability",
        xlabel="-log10(K)",
    ):
...
class PorosityView(GraphViewHisto):
    ...
    def __init__(
        ...
        bins=60,
        color="blue",
        title="A posteriori histogram of the porosity",
    ):
...
class ConductivityView(GraphViewHisto):
    ...
    def __init__(
        ...
        bins=60,
        color="orange",
        title="A posteriori histogram of the thermal conductivity",
    ):
        super().__init__(molomodel, bins, color, title)
...
class CapacityView(GraphViewHisto):
    ...
    def __init__(
        ...
        bins=60,
        color="pink",
        title="A posteriori histogram of the thermal capacity",
    ):
        super().__init__(molomodel, bins, color, title)
