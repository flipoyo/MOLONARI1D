...
class Prior:
    ...
    """
    user_range: object #tuple de bornes physiques ou float si la valeur physique est fixée
    user_sigma: float
    ...
    def __post_init__(self):
        ...
        if self.scale == 'symlog':
            ...
            self.linthresh = max(abs(lower_bound), abs(upper_bound)) / SYMLOG_LINTHRESH_RATIO
    ...
    def physical_to_mcmc(self, physical_value):
        ...
        if self.scale == 'log':
            ...
            return sign * np.where(abs_val > self.linthresh, 
    ...
    def mcmc_to_physical(self, mcmc_value):
        ...
        if self.scale == 'log':
            ...
            return sign * np.where(abs_val > 1, 
    ...
    def sample(self):
        ...
        """
        if self.is_fixed: return self.mcmc_range[0]
    ...
    def __repr__(self) -> str:
        ...
        if self.is_fixed:
            ...
            return f"Prior sur l'intervalle physique [{self.user_range[0]:.2e}, {self.user_range[1]:.2e}]" + scale_info
    ...
    priors = {
        "moinslog10IntrinK": ((11, 15), 0.01),  # (intervalle, user_sigma)
        "n": ((0.01, 0.25), 0.01),
        "lambda_s": ((1, 5), 0.1),
        "rhos_cs": ((1e6, 1e7), 1e5),
        "q_s": ((0, 1), 1e2)
