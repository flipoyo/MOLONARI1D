...
def gelman_R_chap(nb_current_iter, nb_param, nb_layer, chains, threshold=1.1):
    ...
    for l in range(nb_layer):  
        ...
        P1 = np.zeros(nb_param)
        P2 = np.zeros(nb_param)
        P3 = np.zeros(nb_param)
        cov1 = np.zeros(nb_param)
        cov2 = np.zeros(nb_param)
        var_V_chap = np.zeros(nb_param)

        
        for j in range(nb_param):
            P1[j] = ((nb_current_iter - 1)/nb_current_iter)**2 / n_chains * np.var(Var_intra[:,j])
        ...
        R[l] =  np.sqrt(V_chap/var_intra) #* (df + 3) / (df + 1)
