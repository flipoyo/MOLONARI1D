...
void loop() {
  ...
  if(isBefore(start_year0, start_month0, start_day0, start_hour0, start_minute0, start_second0, now.year(), now.month(), now.day(), now.hour(), now.minute(), now.second())&&isBefore(now.year(), now.month(), now.day(), now.hour(), now.minute(), now.second(),end_year0,end_month0,end_day0,end_hour0,end_minute0,end_second0)){
    ...
    #endif //ECHO_TO_SERIAL
    delay(10);
    //we define below the 4 temperatures and the pressure 
    int t1;
    int t2;
    int t3;
    int t4;
    int pressure;
    boolean measure_available = false;//boolean that is initially false and becomes true as soon as we received information from the underwater Arduino
    //int temp;
    radio.begin();//we start radio communication with the underwater Arduino card
    radio.openWritingPipe(adresses[1]);//sends data to the underwater arduino
    radio.openReadingPipe(1,adresses[0]);//receives data form the underwater arduino
    radio.setPALevel(RF24_PA_HIGH);//sets the level of transmission, min for close trans/em and max for far trans/em

    while(!measure_available){
      radio.stopListening();//sets this arduino as emettor
      order = 1;
      radio.write(&order, sizeof(order));//we send the underwater Arduino order = 1; it means the underwater Arduino has to measure temperatures and pressure now and send us these data back
      delay(100);
      //order = 0;
      radio.startListening();//sets this arduino as a receptor
      int i0 = 0;
      //we check if there is any data available from the underwater arduino until we receive data. If we didn't receive data after 10 iterations then we go back to the beginning of the previous loop and send back a signal order=1 to the underwater Arduino
      while(i0<10 && !measure_available){
        //if we receive data, then we read it and set the values we received to the temperatures/pressure variables
        if(radio.available()){
          radio.read(&order, sizeof(order));
          if (order == 0){//order=0 means that the underwater Arduino is in emettor mode and is about to send data
            measure_available = true;
            //Serial.println("boucle if");
            radio.read(&t1,sizeof(t1));//we give the value we received from the underwater Arduino to the t1 variable (first temperature)
    ...
    //now, the variables t1,t2,t3 and t4 and pressure have the values given by the sensors that are in the river. Indeed, the fact that the while loop has stopped means measure_available is true, which means 
    //pressure and temperature values have been received from the underwater arduino and affected to the variables t1,t2,t3,t4 and pressure
    ...
    if(interruption){//if the SD card isn't inserted then write the temperatures in the EEPROM memory to store them waiting the SD card is inserted again
      ...
      Serial.print(", ");
      Serial.print(t2);//write the 2nd temperature in the serial monitor
      Serial.print(", ");    
      Serial.print(t3);
      Serial.print(", ");
      Serial.print(t4);
