...
Après c'est Upload (le bouton avec la flèche). Si l'Arduino n'apparaît pas quand on vous demande de choisir la carte alors que celle-ci est bien branchée, il faut appuyer deux fois sur le bouton reset de l'Arduino. Ensuite, il faut ressélectionner le port COM à droite du bouton upload. Si après ça, ça marche toujours pas : google et bon courage :)
...
*Remarque : Certaines instructions de cette partie ne sont utiles que si vous voulez faire une démo. Pour des tests, vous pouvez vous contenter de brancher les cartes sur un ordinateur en USB, pour avoir une connection en Serial. En particulier, vous pouvez vous passer de mettre l'électronique dans la boite étanche.*
...
Là aussi, si tout se passe bien la LED orange devrait s'allumer puis s'éteindre. Si elle ne s'éteint pas, c'est probablement parce qu'il n'y a aucun logiciel qui utilise le port Serial sur l'ordinateur.  
...
* On a remarqué que le relai fonctionne mieux quand il est aussi alimenté par batterie, même s'il est aussi relié en USB. Peut-être que le cable avait un faux-contact.
